// Vercel serverless function to proxy gas station API requests
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Determine the correct endpoint based on network type
    const isIota = network.toLowerCase().includes('iota');
    const endpoint = isIota 
      ? 'https://gas.movevm.tools/api/iota/sponsor'
      : 'https://gas.movevm.tools/api/sponsor';
    
    // Forward request to the actual gas station API
    const gasStationResponse = await fetch(endpoint, {
      method: 'POST',
      headers: {
